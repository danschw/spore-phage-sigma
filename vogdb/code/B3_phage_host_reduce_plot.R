...
d.sp%>%
  ...
  #           p1 = sum(n.sigma>0)/n,
  #           p2 = sum(n.sigma>1)/n) #%>% 
  ggplot( aes(x = n.sigma,color = phylum)) +
  stat_ecdf( aes(y = 1 - ..y..),geom = "step", size = 2)+
  # stat_ecdf( aes(y = 1 - ..y.., fill=phylum),geom = "area")+
  # stat_ecdf(geom = "point", size = 4, shape=21, fill = "white")+
  # geom_text(data=n.labs, aes(label=lab), x=3,y=0.9)+
  scale_y_continuous(labels=scales::percent) +
  ylab("Phage genomes") +
  xlab("Sigma factors per genome")+
...
ad.test(data = stat.data, n.sigma ~ phylum)


library(MASS)
d <- tibble()

for(phyl in unique(stat.data$phylum)){
  d <- 
  stat.data %>% 
    filter(phylum==phyl) %>% 
    pull(n.sigma) %>% 
    fitdistr( densfun = "negative binomial") %>% 
    broom::tidy() %>% 
    mutate(phylum = phyl) %>% 
    bind_rows(d,.)
...
firmi%>%
  ...
  # group_by(genus.plot, n.sigma) %>% 
  # summarise(count = n()) %>% 
  ggplot( aes(x = n.sigma,color = genus.plot)) + 
