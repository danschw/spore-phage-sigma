...
rax.fbp <- read.tree(here("phylo/data/reduced_set_to_align","tree-all.raxml.supportFBP"))
d.rax.fbp <- as_tibble(rax.fbp)

# Best ML tree with Transfer bootstrap (TBE) support values
rax.tbe <- read.tree(here("phylo/data/reduced_set_to_align","tree-all.raxml.supportTBE"))
...
d.rax %>% 
  ...
  geom_nodepoint(aes(fill=support, shape = support), size=1.5)+
